...
export function Agents() {
  ...
  if (loading) {
    return <LoadingState label="جاري تحميل الوكلاء..." />;
  }

  if (agents.length === 0) {
    return <EmptyState icon={Users} label="لا يوجد وكلاء" sublabel="لم يتم تحميل الوكلاء من الخادم" />;
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-2 mt-4 flex-wrap">
          <button
            className={`text-xs px-3 py-1.5 rounded-lg transition-all ${
              filter === '' ? 'bg-accent/20 text-accent border border-accent/30' : 'bg-elevated/50 text-primary-secondary hover:bg-hover'
            }`}
            onClick={() => setFilter('')}
          >
            الكل
          ...
          {specs.map((s) => (
            <button
              key={s.name}
              className={`text-xs px-3 py-1.5 rounded-lg transition-all ${
                filter === s.name ? 'bg-accent/20 text-accent border border-accent/30' : 'bg-elevated/50 text-primary-secondary hover:bg-hover'
              }`}
      ...
      {Object.entries(specGroups).map(([specName, specAgents]) => {
        ...
        return (
          <div key={specName} className="animate-slide-up">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-primary-muted font-mono mr-auto">{specName}</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {specAgents.map((agent) => (
                <AgentCard
                  key={agent.name}
                  agent={agent}
                  isExternal={agent.specialization === 'external'}
                  onRemove={() => void handleRemove(agent.name)}
                />
              ))}
            </div>
          </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowModal(false)} className="text-primary-muted hover:text-primary transition-colors">
                <X size={20} />
              </button>
            </div>
            <p className="text-xs text-primary-muted mb-4 flex items-center gap-1">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-semibold text-primary-secondary mb-1.5">الاسم</label>
                <input
              ...
              <div>
                <label className="block text-sm font-semibold text-primary-secondary mb-1.5">URL</label>
                <input
              ...
              <div>
                <label className="block text-sm font-semibold text-primary-secondary mb-1.5">النموذج (اختياري)</label>
                <input
                  className="form-input font-mono text-sm"
                  placeholder="llama3"
              ...
              <div>
                <label className="block text-sm font-semibold text-primary-secondary mb-1.5">API Key (اختياري)</label>
                <input
                  className="form-input font-mono text-sm"
                  type="password"
                  placeholder="sk-..."
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button className="btn-primary flex-1" onClick={() => void handleRegister()}>
                  إضافة
                </button>
              </div>
            </div>
  ...
  return (
    <div className="glass-card-hover p-4 group">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-elevated/30 rounded-lg py-2">
          <div className="text-[10px] text-primary-muted">مكتملة</div>
          <div className="text-sm font-bold text-success">{agent.stats?.tasks_completed || 0}</div>
        </div>
        <div className="bg-elevated/30 rounded-lg py-2">
          <div className="text-[10px] text-primary-muted">فشلت</div>
          <div className="text-sm font-bold text-error">{agent.stats?.tasks_failed || 0}</div>
        </div>
        <div className="bg-elevated/30 rounded-lg py-2">
          <div className="text-[10px] text-primary-muted">الوقت</div>
          <div className="text-sm font-bold text-accent">{(agent.stats?.total_time_s || 0).toFixed(1)}s</div>
        </div>
      </div>
