...
export function Specializations() {
  ...
  if (loading) {
    return <LoadingState label="جاري تحميل التخصصات..." />;
  }
      ...
      <div className="space-y-4 animate-fade-in">
        ...
        <div className="glass-card p-6">
          <div className="flex items-center gap-4 mb-6">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-accent/20 to-blue-600/20 border border-accent/30 flex items-center justify-center">
              <Icon size={26} className="text-accent" />
            </div>
            <div className="flex-1">
              <h2 className="text-xl font-bold text-white">{SPECIALIZATION_LABELS[spec?.name || ''] || spec?.name}</h2>
              <p className="text-sm text-primary-secondary">{spec?.description}</p>
            </div>
            {spec?.tool && (
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-primary-secondary mb-2">المهمة / الطلب</label>
              <textarea
            ...
            {showDataField && (
              <div>
                <label className="block text-sm font-semibold text-primary-secondary mb-2">
            ...
            >
              {submitting ? (
                <>
                  <span className="dot-pulse">جاري الإرسال...</span>
                </>
              ) : (
                <>
            ...
            {result && (
              ...
              >
                ...
                {result.ok && (
                  <div className="text-xs mt-1 text-success/70">تتبع المهمة في تبويب "المهام"</div>
                )}
              </div>
            )}
          </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-primary-muted">{specs.length} تخصص · اختر تخصصاً لإرسال مهمة</p>
      </div>
        ...
        {specs.map((spec, i) => {
          ...
          return (
            <button
              key={spec.name}
              onClick={() => {
                setSelected(spec.name);
                setResult(null);
              }}
              className="glass-card-hover p-5 text-right group animate-slide-up"
              style={{ animationDelay: `${i * 30}ms` }}
            >
              <div className="flex items-start gap-3 mb-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-accent/15 to-blue-600/15 border border-accent/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                  <Icon size={22} className="text-accent" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-white text-sm">{SPECIALIZATION_LABELS[spec.name] || spec.name}</div>
                  <div className="text-xs text-primary-muted font-mono">{spec.name}</div>
                </div>
              </div>
              <p className="text-xs text-primary-secondary mb-3 line-clamp-2 leading-relaxed">{spec.description}</p>
              <div className="flex items-center gap-2 text-xs">
                <span className="text-primary-muted">{spec.agents} وكلاء</span>
                {spec.tool && (
                ...
                <ArrowRight size={14} className="text-accent mr-auto opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </button>
          );
        })}
